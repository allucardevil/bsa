                	...
                	<div class="post">
                        ...
                        <div class="textBlogclassic">
                            <h3>
								...
								if ($row->wc_type == 'news')
								{ 
									echo anchor('' . $row->wc_lang . '/' . $row->wc_type . '/' . $row->wc_sub_category . '/' . $row->wc_id . '/' . dash($row->wc_title)  . '/',$row->wc_title);
								}
								else
								{
									echo anchor('' . $row->wc_lang . '/' . $row->wc_category . '/' . $row->wc_sub_category . '/' . $row->wc_id . '/' . dash($row->wc_title)  . '/',$row->wc_title);
								}
                            ...
                            <p>
								...
								{
									echo anchor('' . $row->wc_lang . '/corporate/' . $row->wc_sub_category . '/' . $row->wc_id . '/'. dash($row->wc_title) . '/', 'Read More', array('class' => 'buttonPro'));
								}
								elseif(($row->wc_type == 'news'))
								{
									echo anchor('' . $row->wc_lang . '/news/' . $row->wc_sub_category . '/' . $row->wc_id . '/'. dash($row->wc_title) . '/', 'Read More', array('class' => 'buttonPro'));
								}
								?>
                            </p>
                            <p></p>	
